...
function ConceptIdea() {
  ...
  const conceptIdea = posts.filter((post) => post.category === "Concept Idea");
  return (
    <div className="py-8">
      <h1 className="font-bold text-3xl md:text-4xl mb-6">Concept Idea</h1>
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-1">
        {conceptIdea.map((post) => (
          <Link
            key={post.id}
